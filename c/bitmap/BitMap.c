...
int offset;           //bytes from the beginning to the image偏移量
int width, height;    //image width & height 图像的宽度与高度
int RowSize;
color buf[1000][1000];
char FillorNot[1000][1000];
...
/* return ipart(x + 0.5) */
/* 将一个double型的数字先加上0.5， 然后转化成int返回 */
...
/* Set the color of a specified pixel */
/* 设置一个指定点的颜色,  使用RGB颜色 */
int plot(int x, int y, char R, char G, char B)
{
...
/* Set the color of a specified pixel not using RGB, but the lightness */
/* 设置一个指定点的颜色， 不使用RGB， 而是亮度                         */
int plotL(int x, int y, double light)
{
    int tmp;

    tmp = (1.0 - light) * 255;
    lseek(fd, offset + y * RowSize + x * 3, SEEK_SET);
    write(fd, &tmp, 1);
    write(fd, &tmp, 1);
    write(fd, &tmp, 1);
    return 0;
...
/* draw a line with AA Tec                    */
/* 从网上找到的画线算法， 只支持宽度为1的线条 */
void drawLine_aa(double x1, double y1, double x2, double y2)
{
    ...
    xpxl1 = xend;
    ypxl1 = ipart(yend);
    bright1 = rfpart(yend) * xgap;
    bright2 = fpart(yend) * xgap;
    plotL((int)xpxl1, (int)ypxl1, bright1);
    plotL((int)xpxl1, (int)ypxl1 + 1, bright2);
    ...
    bright1 = rfpart(yend) * xgap;
    bright2 = fpart(yend) * xgap;
    plotL((int)xpxl2, (int)ypxl2, bright1);
    plotL((int)xpxl2, (int)ypxl2 + 1, bright2);

    bright1 = rfpart(intery);
    bright2 = fpart(intery);
    for (i = xpxl1 + 1; i<= xpxl2 - 1; ++i) {
        plotL((int)i, ipart(intery), (int)(bright1));
        plotL((int)i, ipart(intery) + 1, (int)(bright2));
        intery += gradient;
    }
}
...
/* 初始化边界数组 */
void init_boundary(int size, int up[], int down[])
{
    int i;
...
void draw_line(vertex s, vertex e, color cl, double lwidth)
{
    double k;
    vertex vt[4];
...
int main()
{
    ...
    fd = open("test.bmp", O_RDWR | O_CREAT | O_TRUNC, 0644);
    ...
    scanf("%lf", &weight);
    scanf(" (%d, %d, %d)", &(cl.R), &(cl.G), &(cl.B));
    scanf(" (%d, %d, %d)", &(cl2.R), &(cl2.G), &(cl2.B));
    scanf("%d", &flag);
    /* 数据读入结束 */
